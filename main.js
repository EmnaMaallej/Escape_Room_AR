...
// === CAMÉRA POV (First Person) ===
const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
camera.position.set(0, 1.7, 5);  // Hauteur des yeux (1.7m)
...
// Demander le verrouillage de la souris au clic
...
// Détecter le verrouillage/déverrouillage
...
function updateMovement() {
    ...
    // Calculer la direction en fonction des touches
    if (keys.forward) direction.z -= 1;
    if (keys.backward) direction.z += 1;
    if (keys.left) direction.x -= 1;
    if (keys.right) direction.x += 1;

    // Normaliser pour éviter le déplacement plus rapide en diagonal
    if (direction.length() > 0) {
        direction.normalize();
    }

    // Appliquer la rotation de la caméra (uniquement horizontale)
    ...
    // Limites de la pièce (ne pas traverser les murs)
...
// Initialiser le Puzzle 1
const puzzle1 = new ClockPuzzle(scene, camera, renderer, clockGroup, hourHand, minuteHand);
...
const sofaFabric = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.8 });
const sofaWood = new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.6 });

const sofaSeat = new THREE.Mesh(new THREE.BoxGeometry(2.8, 0.6, 1.1), sofaFabric);
sofaSeat.position.y = 0.6;
sofaSeat.castShadow = true;
sofaGroup.add(sofaSeat);

const sofaBack = new THREE.Mesh(new THREE.BoxGeometry(2.8, 1.1, 0.25), sofaFabric);
...
[-0.7, 0.7].forEach(x => {
    const seatCushion = new THREE.Mesh(seatCushionGeom, sofaFabric);
    seatCushion.position.set(x, 0.75, 0);
    seatCushion.castShadow = true;
    sofaGroup.add(seatCushion);
});

const armLeft = new THREE.Mesh(new THREE.BoxGeometry(0.25, 1.0, 1.1), sofaFabric);
armLeft.position.set(-1.5, 1.2, 0);
armLeft.castShadow = true;
sofaGroup.add(armLeft);

const armRight = new THREE.Mesh(new THREE.BoxGeometry(0.25, 1.0, 1.1), sofaFabric);
armRight.position.set(1.5, 1.2, 0);
armRight.castShadow = true;
sofaGroup.add(armRight);

const footGeom = new THREE.CylinderGeometry(0.06, 0.06, 0.25, 12);
[[-1.3, 0.125, 0.45], [1.3, 0.125, 0.45], [-1.3, 0.125, -0.45], [1.3, 0.125, -0.45]].forEach(pos => {
    const foot = new THREE.Mesh(footGeom, sofaWood);
